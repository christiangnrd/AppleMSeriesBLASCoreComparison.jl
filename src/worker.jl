# DGEMM timing kernel and worker entry point for the E-core/P-core/Super-core sweep.
#
...
Run a single (nthreads, QoS-mode, perf-level) measurement point configured by
the `BENCH_*` entries of `env` and print one CSV row on stdout.
"""
